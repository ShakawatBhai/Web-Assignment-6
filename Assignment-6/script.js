...
function showLatest(data){
   data.forEach(detum =>{
       ...
       newPost.innerHTML =  `
           <div class="m-2 xl:max-w-sm bg-white border rounded-lg  border-l-4">
               <div class = "">
    ...
    if(typeof data === 'object'){
        ...
        data.posts.forEach(detum => {
            ...
            if(readId.find(item => item == detum.id)){
                btn = "<button class = 'flex items-center  gap-2 text-red-500 font-bold text-xl rounded-xl'></button>"
            }
            else{
                btn = `<button id='btn-${detum.id}' onclick='addPost(${detum.id},${detum.view_count})' class='show-btn flex items-center gap-2 text-green-500 font-bold text-xl rounded-xl'><img class='w-[40px] h-[30px] bg-green-500 rounded-full p-2' src='assets/envelope-open-regular.svg' alt=''></button>`;
            }

            Data.innerHTML = `
                <div class="xl:flex px-2  w-full my-4 rounded-xl border-b-4 border-l-2">
            
                    <div class="relative inline-block">
                        <div class="absolute top-0 left-20 mt-6 w-4 h-4 ${color} rounded-full"></div>
                        <img src="${detum.image}" alt="hello" class="mt-8 ml-4  w-[80px] h-[60px] rounded-lg">
                    </div>
...
function addPost(id,views){
    ...
    if(Btn.classList.contains('text-green-500')){
        ...
        readPost.innerHTML = `
            <div class = "xl:flex justify-between bg-white p-4 my-4 rounded-lg  border-b-4 border-l-2 ">
